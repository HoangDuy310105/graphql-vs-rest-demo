...
### Giai đoạn 5: (Tùy Chọn) Demo tính năng "Thêm mới" (Mutation)
*Nếu Giáo viên hỏi: "Thế bây giờ muốn Thêm dữ liệu (Tạo bài viết mới) thì GraphQL có làm được không, hay chỉ biết lấy dữ liệu (query)?"*

Anh có thể tự tin biểu diễn luôn:

*   **Về phía REST:** Anh giải thích là mình phải tạo ra một đường dẫn API hoàn toàn mới (ví dụ: `POST /api/authors/1/posts`) để tiếp nhận dữ liệu báo lên Server.
*   **Về phía GraphQL:** Anh mở lại tab `http://localhost:8080/graphiql`, không cần mở URL mới nào cả, xóa trắng ô bên trái và gõ đoạn lệnh (Mutation) như sau:

```graphql
mutation {
  createPost(title: "Bài viết mới 100", content: "Nội dung bài viết", authorId: "1") {
    id
    title
  }
}
...
> **(Lời thoại chốt hạ):** *"Dạ thưa cô, tính năng Đọc dữ liệu gọi là Query, còn tính năng Ghi/Tạo dữ liệu gọi là Mutation. 
> GraphQL vô cùng ngầu ở chỗ: Nó vẫn dùng chung 1 cái đường dẫn ban đầu, không bắt Backend đẻ thêm Endpoint mới.
> Chẳng những nó thêm được dữ liệu vào Database, mà ngay trong lúc đó nó tự động lấy luôn ID và Title của bài viết vừa được tạo xong để trả về cho Client chỉ bằng 1 câu lệnh duy nhất. Sạch sẽ và nguy hiểm hơn REST rất nhiều ạ!"*
